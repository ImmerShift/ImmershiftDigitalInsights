      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden w-full relative">
        ...
        <header className="h-16 bg-white border-b border-[#EAE3D9] flex items-center justify-between px-4 sm:px-8 shrink-0 z-40">
          <div className="flex items-center gap-4 sm:gap-6">
            ...
            <div className="hidden sm:flex gap-2">
              <button 
                ...
                className={`px-2 py-1 rounded text-[10px] font-black tracking-widest uppercase transition-colors ${language === 'en' ? 'bg-[#3E1510] text-white' : 'text-[#856B66] hover:bg-[#F9F7F4]'}`}
              >
                EN
              </button>
              <button 
                ...
                className={`px-2 py-1 rounded text-[10px] font-black tracking-widest uppercase transition-colors ${language === 'id' ? 'bg-[#3E1510] text-white' : 'text-[#856B66] hover:bg-[#F9F7F4]'}`}
              >
                ID
              </button>
            </div>
          ...
          <div className="flex items-center gap-4">
            ...
            {/* Hover-only tooltip, so it does nothing on touch and it pushed the
                header 18px past a 375px viewport. Desktop only. */}
            <button className="hidden sm:block p-2.5 bg-[#FDF8F3] border border-[#EAE3D9] rounded-xl text-[#3E1510] relative group">
        ...
        {/* Main Content Area */}
        {/* pb-28 reserves room for the fixed "Ask AI Analyst" button, which
            otherwise sits on top of whatever primary action ends the page. */}
        <main id="main-data" tabIndex={-1} className="flex-1 overflow-y-auto relative bg-[#F9F7F4] focus:outline-none pb-28">
          ...
          {currentView === 'users' && (
            <div className="flex items-center justify-center p-20 text-[#856B66] font-bold uppercase tracking-widest text-xs h-full">
